...
switch ($accion) {
    case "btnEstado":
        ...
        $comprobarActivo = $pdo->prepare("SELECT * FROM alimentos_y_bebidas WHERE id=:id");
        $comprobarActivo->bindParam(':id', $txtId);
        $comprobarActivo->execute();
        $alimentos = $comprobarActivo->fetch(PDO::FETCH_ASSOC);

        $elementoActivo = $alimentos['activo'];

        // Si el establecimiento está inactivo entonces se actualizará como habilitado(1)
        if ($elementoActivo == 0) {
            $sentencia = $pdo->prepare("UPDATE alimentos_y_bebidas SET activo=1 WHERE id=:id");
            ...
            $sentencia = $pdo->prepare("UPDATE alimentos_y_bebidas SET activo=0 WHERE id=:id");
...
if ($eliminar) {
    ...
    if ($sentencia) {
        echo '<script language="javascript">alert("Registro eliminado correctamente");window.location.href="alimentosYBebidas.php"</script>';
    }
                ...
                <div class="container mt-1 pt-3 px-2">
                    ...
                    <div class="row mt-3 ">
                        <table class="table order-table">
                            ...
                            <?php foreach ($listaAlimentos as $alimentos) { ?>
                                <?php
                                $c = $c + 1;
                                if ($alimentos['activo']) { ?>
                                    <tr>
                                        <td class="text-center align-middle"><?php echo $c; ?></td>
                                        <td class="text-center align-middle"> <a class="text-decoration-none text-dark" title="Ver información" href="ver_alimento.php?id=<?php echo $alimentos['id']; ?>"><?php echo $alimentos['nombre']; ?></a></td>
                                        <td style="width: 35%;" class="text-center align-middle"><?php echo $alimentos['direccion']; ?></td>
                                        <td style="width: 24%;" class="text-center align-middle"><?php echo $alimentos['telefono']; ?></td>
                                        <td style="width: 20%;" class="text-center align-middle">

                                            <form action="" method="post">
                                                <input type="hidden" name="txtId" value="<?php echo $alimentos['id']; ?>">
                                                <input type="hidden" name="txtNombre" value="<?php echo $alimentos['nombre']; ?>">
                                                <input type="hidden" name="txtDireccion" value="<?php echo $alimentos['direccion']; ?>">
                                                <input type="hidden" name="txtTelefono" value="<?php echo $alimentos['telefono']; ?>">

                                                <a style="font-size: 13px;" class="btn btn-primary text-center m-1" title="Ver información" href="ver_alimento.php?id=<?php echo $alimentos['id']; ?>"><i class="fas fa-eye"></i></a>
                                                <a style="font-size: 13px;" class="btn btn-success text-center m-1" title="Editar" href="actualizar.php?id=<?php echo $alimentos['id']; ?>"><i class="fas fa-edit"></i></a>
                                                <a href='#' style="font-size: 13px;" class="btn btn-danger m-1" title="Eliminar permanentemente" onclick="preguntar(<?php echo $alimentos['id'] ?>)"><i class="fas fa-trash-alt"></i></a>
                                ...
                                <?php } else { ?>
                                    <tr>
                                        ...
                                        <td class="text-muted text-center"><?php echo $alimentos['nombre']; ?> (deshabilitado)</td>
                                        <td class="text-muted text-center"><?php echo $alimentos['direccion']; ?></td>
                                        <td class="text-muted text-center"><?php echo $alimentos['telefono']; ?></td>
                                        <td class="text-center">
                                            ...
                                            <form action="" method="post">
                                                ...
                                                <a style="font-size: 13px;" href='#' type="submit" title="Eliminar permanentemente" class="btn btn-danger" onclick="preguntar(<?php echo $alimentos['id'] ?>)"><i class="fas fa-trash-alt"></i></a>
